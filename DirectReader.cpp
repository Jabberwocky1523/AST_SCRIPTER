...
int DirectReader::getRegisteredCompressionType(const char *file_name)
{
    ...
    while (reg)
    {
        if (!strcmp(capital_name, reg->ext))
            return reg->type;
...
size_t DirectReader::decodeLZSS(struct ArchiveInfo *ai, int no, unsigned char *buf)
{
    ...
    while (count < ai->fi_list[no].original_length)
    {
        ...
        {
            if ((c = getbit(ai->file_handle, 8)) == EOF)
                break;
            buf[count++] = c;
            decomp_buffer[r++] = c;
            r &= (N - 1);
        ...
        {
            if ((i = getbit(ai->file_handle, EI)) == EOF)
                break;
            if ((j = getbit(ai->file_handle, EJ)) == EOF)
                break;
            for (k = 0; k <= j + 1; k++)
            {
