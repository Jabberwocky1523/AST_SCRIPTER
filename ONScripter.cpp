...
#else
    window = SDL_CreateWindow("Test", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, screen_width, screen_height, DEFAULT_VIDEO_SURFACE_FLAG);
...
void ONScripter::flushDirect(SDL_Rect &rect, int refresh_mode)
{
    ...
    refreshSurface(accumulation_surface, &rect, refresh_mode);
    SDL_Rect dst_rect = rect;
    if (AnimationInfo::doClipping(&dst_rect, &screen_rect) || (dst_rect.w == 0 && dst_rect.h == 0))
        return;
    SDL_BlitSurface(accumulation_surface, &dst_rect, screen_surface, &dst_rect);
    // SDL_UpdateRect(screen_surface, &dst_rect.x, &dst_rect.y, screen_bpp);
